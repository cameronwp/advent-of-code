...
def part2():
    ...
    for line in open_puzzle_input_and_loop(day=6):
        elements = line.split()
        if i == 0:
            time_total = time_total + "".join(elements[1:])
        else:
            distance = distance + "".join(elements[1:])
        i += 1

    time_total = int(time_total)
    distance = int(distance)

    lower, upper = quadratic_equation(-1, time_total, distance)
    return np.floor(upper) - np.floor(lower + 1) + 1
